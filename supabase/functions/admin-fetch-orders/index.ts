...
serve(async (req) => {
  ...
  try {
    ...
    const isValid = await validateToken(body.token);
    if (!isValid) {
      console.log("Admin fetch attempt: failed - invalid or expired token");
      return new Response(
    ...
    console.log(`Admin fetch orders: success - ${data.length} orders returned`);
    ...
    console.error("Error in admin-fetch-orders:", error);
    const origin = req.headers.get("origin");
    return new Response(
